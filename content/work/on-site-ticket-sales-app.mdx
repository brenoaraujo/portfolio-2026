...
<TLDR>
On game day, raffle sellers are often volunteers or temporary staff who get only a few minutes of training before they start selling in a crowded, high-pressure environment. The app needed to be learnable at a glance, fast enough to keep a line moving, and flexible enough for different events, charities, and brand contexts. I redesigned the Android POS experience around focused, one-action-per-screen flows, clear feedback after every action, and white-label components that production could adapt without developer support. We also added operational improvements like in-app chat for faster event support and QR-code ticket checking to reduce receipt printing delays and paper waste, making the experience easier for both sellers and participants.
</TLDR>
...
<CaseSection title="Challenges">
  <Prose>
    One of the biggest challenges was **designing a ticketing system for people with minimal training**. Some have to learn the basic rules of a 50/50 raffle minutes before they started selling. To support them we focus on:
